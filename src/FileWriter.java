...
public class FileWriter {
    ...
    private static final String DEFAULT_FILENAME = "out.txt"; // Subject to
                                                              // change
    ...
    static {
        try {
            writer = new BufferedWriter(
                    new java.io.FileWriter(DEFAULT_FILENAME));
        } catch (IOException e) {
            e.printStackTrace();
        }
        writeThread.start();
    ...
    private static void continuouslyWrite() {
        String line;
        // Nand check to make sure that the queue is empty before terminatint
        // the
        // thread.
        while (!(buffer.isEmpty() && finished)) {
            line = buffer.poll();
            if (line == null) {
                // Yield if there's nothing to write, would use blocking queue
                // but then
                // there's the problem of terminating the thread at the end of
                // the
                // program.
                Thread.yield();
            } else {

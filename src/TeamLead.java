...
public class TeamLead extends Actor {
    /*
     * Inherited from Actor protected Clock clock; protected ConferenceRoom
     * conferenceRoom;
     */
    ...
    public void run() {
        ...
        lunchLength = 30 + (int) (Math.random() * (30 - clock
                .convertSimulated((int) arrivalTime)));
    ...
    protected void idleUntil(int relativeMinutes) {
        ...
        while (clock.convertSimulated((int) clock.getTimePassedMillis()) < relativeMinutes) {
            // Check if should go to lunch.
            if (!hasHadLunch) {
                // Currently Just checks if there's enough time to have lunch.
                if ((relativeMinutes - clock.convertSimulated((int) clock
                        .getTimePassedMillis())) > lunchLength) {
                    // Go on lunch break
                    busy(lunchLength);
                    lunch += lunchLength;
                }
        ...
        synchronized (questionLock) {
            ...
            /*
             * while(!questionBuffer.isEmpty()) { questionBuffer.poll();
             * flushCount++; }
             */
        }
        for (int i = 0; i < flushCount; i++) {
            try {
                answerBuffer.offer(-1, 2, TimeUnit.MILLISECONDS);
    ...
    private ConcurrentLinkedQueue<Developer> questionBuffer = new ConcurrentLinkedQueue<Developer>();
    // Guarantees fairness.
    private SynchronousQueue<Integer> answerBuffer = new SynchronousQueue<Integer>(
            true);
    // Used to make sure that no Developers ask questions after the team lead
    // stops taking them
